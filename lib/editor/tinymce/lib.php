...
class tinymce_texteditor extends texteditor {
    ...
    protected function get_init_params($elementid, array $options=null) {
        ...
        //*************Hack to enable the**************** 
        //**installation of other TinyMCE modules********
        //**Author: Patrick Thibaudeau
        //**Company: oohoo.biz
        //**copyright: 2011
        $defaultplugins = $CFG->mceconf_default_plugins;
        
        if (!empty($CFG->mceconf_plugins)){
            $mceplugins = ",$CFG->mceconf_plugins";
        } else {
            $mceplugins = '';
        }
        if (!empty($CFG->mceconf_buttons1_add)){
            $mcerow1 = ",|,$CFG->mceconf_buttons1_add";
        } else {
            $mcerow1 = '';
        }
        if (!empty($CFG->mceconf_buttons2_add)){
            $mcerow2 = ",|,$CFG->mceconf_buttons2_add";
        } else {
            $mcerow2 = '';
        }
        if (!empty($CFG->mceconf_buttons3_add)){
            $mcerow3 = ",|,$CFG->mceconf_buttons3_add";
        } else {
            $mcerow3 = '';
        }
